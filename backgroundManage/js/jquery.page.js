...
(function($){
	...
	function Page(container,fn,args,style){
		...
		var style=this.style=$.extend({
			"fontSize":12,//字体大小
			"width":700,//页码盒子总宽度
			"height":26,//页码总高度，默认20px
			"pagesMargin":6,//每个页码或按钮之间的间隔
			"paddL":0,//左边留白
			"paddR":0,//右边留白
			"borderColor":"#2f7bc4",//边线颜色
			"currentColor":"#02dbb7",//当前页码的字体颜色
			"disableColor":"#fff",//不可点击按钮的字体颜色
			"disableBackColor":"#084f98",//不可点击按钮的背景色
			"prevNextWidth":68,//上页下页的宽度
	...
	Page.prototype.setStyle=function(){
		...
		this.obj.children().css({'float':'left','margin-left':marLR+'px','text-align':'center','color':'#fff'});
		...
		this.obj.find('a.turndown').css({'margin-left':'5px','display':'inline-block','width':'35px','height':s.height-2+'px','line-height':s.height-2+'px'});
		...
		this.obj.find('input').css({'outline':'none','border':'1px solid #2b6cae','height':s.height-2+'px','line-height':s.height-2+'px','width':'30px',margin: '0 5px','border-radius': '5px','background':'none','color':'#fff','text-align': 'center'});
